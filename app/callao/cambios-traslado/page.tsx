...
export default function CambiosTrasladoPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            await refreshHistorialTraslados();
            setLoading(false);
        };
        loadData();
    ...
    const cargas = useMemo(() => {
        const map = new Map<string, typeof state.cambiosTraslado>();
        state.cambiosTraslado.forEach(c => {
    ...
    const handleExportExcel = () => {
        ...
        exportToExcel(exportRows, columns, `Cambios_Traslado_${new Date().toISOString().split('T')[0]}`);
    };
    ...
    const handleExportPDF = () => {
        ...
        exportToPDF(exportRows, columns, `Cambios_Traslado_${new Date().toISOString().split('T')[0]}`, 'Cambios de Traslado');
    };
                        ...
                        <div className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                paginatedCargas.map((carga, idx) => {
                                    ...
                                    const cambiosTotales = carga.detalles.length;
                                    const motivosTotales = carga.detalles.filter(d => (d.motivoCambio || '').trim().length > 0).length;
                                    return (
                                        <div key={cargaKey} className="px-3 sm:px-4">
                                            ...
                                            >
                                                <div className="flex items-start sm:items-center gap-3 min-w-0 w-full">
                                                    ...
                                                    <div className="hidden sm:flex items-center gap-3 flex-wrap text-[11px] font-semibold text-gray-900 min-w-0">
                                                ...
                                                <div className="hidden sm:flex items-center gap-4 text-[10px] text-gray-600 whitespace-nowrap flex-shrink-0">
                                                    <span className="inline-flex items-center gap-2">
                                                        ...
                                                        <span className="font-bold text-gray-900">{cambiosTotales}</span> cambios
                                                    </span>
                                                </div>
                                            </div>
                                                                ...
                                                                <tbody>
                                                                    {carga.detalles.map((c, i) => (
                                                                        <tr key={`${c.id}-${i}`} className="border-t border-gray-100 text-[11px] hover:bg-blue-50/30 transition-colors">
                                                                            ...
                                                                            <td className="px-4 py-3 text-center text-gray-700 font-semibold">{c.cantidadAnterior ?? '-'}</td>
                                                                            <td className="px-4 py-3 text-center text-gray-700 font-semibold">{c.cantidad}</td>
