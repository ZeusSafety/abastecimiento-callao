...
export default function CambiosEntradaPage() {
    ...
    return (
        <div>
            <div className="mb-5">
                <h1 style={{ fontSize: 20, fontWeight: 700, color: '#002D5A', margin: 0 }}>Cambios de Entrada</h1>
                <p style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>Historial de todos los registros de entradas que fueron modificados</p>
            </div>
                        ...
                        <tbody>
                            {paginated.map((c, i) => (
                                <tr key={`${c.id}-${i}`}>
                                    ...
                                    <td>{c.producto}</td>
                                    <td><span className="badge badge-entrada" style={{ fontSize: 10 }}>{c.operacion}</span></td>
                                    <td style={{ fontSize: 11 }}>{c.almacenSalida}</td>
                                    <td style={{ fontSize: 11 }}>{c.almacenIngreso}</td>
                                    <td>{c.operador}</td>
                                    <td style={{ textAlign: 'center', fontWeight: 600 }}>{c.cantidad}</td>
                                    <td><span className="badge badge-entrada" style={{ fontSize: 10 }}>{c.unidadMedida}</span></td>
                                    <td>{c.entregado}</td>
